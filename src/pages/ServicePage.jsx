...
// Палитра акцентов по имени услуги (meta.background) — пока эвристика на клиенте,
// т.к. API не отдаёт готовый hex. Можно заменить на карту brand-цветов из CMS.
const ACCENT_FALLBACK = '#5be7a9';
...
export default function ServicePage({ type: typeProp, id: idProp } = {}) {
  ...
  //       document.documentElement.style.setProperty('--accent-live', ACCENT_FALLBACK);
  ...
  return (
    <main>
      <Header />
      {/* <ContentSection bodyHtml={parsed.bodyHtml} pdfUrl={parsed.pdfUrl} />
      <Gallery
      ...
      />
      <RelatedServices items={parsed.related} currentTitle={post.title} />
