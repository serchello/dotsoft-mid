...
function renderText(block, index) {
  ...
  const hasYoutube = cleanHtml.includes('youtube.com/watch') || cleanHtml.includes('youtu.be');
  ...
  if (hasYoutube) {
    const youtubeMatch = cleanHtml.match(/https?:\/\/[^\s]*youtube\.com\/watch\?v=[^\s&]+/);
    const youtubeUrl = youtubeMatch ? youtubeMatch[0] : '';
    const videoId = youtubeUrl ? extractYoutubeId(youtubeUrl) : '';
